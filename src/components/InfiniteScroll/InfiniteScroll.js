...
export default class InfiniteScroll extends Component {
  ...
  componentDidMount () {
    ...
    this.el.addEventListener('scroll', this.throttledOnScrollListener, this.supportsPassive ? { passive: true } : false)
    ...
    if (this.props.pullDownToRefresh) {
      this.el.addEventListener('touchstart', this.onStart, this.supportsPassive ? { passive: true } : false)
      this.el.addEventListener('touchmove', this.onMove, this.supportsPassive ? { passive: true } : false)
      this.el.addEventListener('touchend', this.onEnd, this.supportsPassive ? { passive: true } : false)

      this.el.addEventListener('mousedown', this.onStart, this.supportsPassive ? { passive: true } : false)
      this.el.addEventListener('mousemove', this.onMove, this.supportsPassive ? { passive: true } : false)
      this.el.addEventListener('mouseup', this.onEnd, this.supportsPassive ? { passive: true } : false)
  ...
  componentDidUpdate (prevProps) {
    ...
    if (this.props.key === prevProps.key && this.props.dataLength === prevProps.dataLength) return
  ...
  onEnd (evt) {
    ...
    requestAnimationFrame(() => {
      // this._infScroll
      if (this._infScroll) {
          this._infScroll.style.overflowY = 'auto'
          this._infScroll.style.overflowX = 'hidden'
  ...
  isElementAtBottom (target, scrollThreshold = 0.75) {
    const clientHeight =
